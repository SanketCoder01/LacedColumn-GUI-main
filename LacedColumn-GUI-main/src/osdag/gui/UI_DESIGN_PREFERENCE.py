...
class Ui_Form(QDialog):
    ...
    def setupUi(self, DesignPreferences, main, input_dictionary, save_changes_list):
        self.save_changes_list = save_changes_list
        self.main = main
        ...
        for tab_details in main.tab_list():
            ...
            if tab_type == TYPE_TAB_1:
                ...
                r = 1
                grid = QGridLayout(scrollAreaWidgetContents)  # Pass parent widget
                ...
                r = 1
                grid = QGridLayout()  # Create grid layout without parent
                grid.setHorizontalSpacing(10)
                grid.setVerticalSpacing(10)
                grid.setAlignment(Qt.AlignTop|Qt.AlignLeft)
                horizontalLayout.addLayout(grid)  # Add grid to horizontal layout
            ...
            elif tab_type == TYPE_TAB_4:
                tab_weld = QWidget()
                self.tabWidget.addTab(tab_weld, tab_name)
                tab_index += 1
                self.tabWidget.tabs.setTabText(tab_index, tab_name)
                tab_weld.setObjectName(tab_name)

                layout_weld = QVBoxLayout(tab_weld)

                scroll_weld = QScrollArea(tab_weld)
                scroll_weld.setWidgetResizable(True)
                weld_contents = QWidget()
                scroll_weld.setWidget(weld_contents)
                layout_weld.addWidget(scroll_weld)

                vertical_weld = QVBoxLayout(weld_contents)
                horizontal_layout_weld = QHBoxLayout()
                vertical_weld.addLayout(horizontal_layout_weld)

                grid_weld = QGridLayout()
                grid_weld.setAlignment(Qt.AlignTop | Qt.AlignLeft)
                grid_weld.setHorizontalSpacing(10)
                grid_weld.setVerticalSpacing(10)
                horizontal_layout_weld.addLayout(grid_weld)

                r = 1
                title = QLabel("Weld Design Preferences")
                title.setObjectName("_title")
                grid_weld.addWidget(title, r, 1)
                r += 2

                elements = tab_elements(main, input_dictionary)
                for element in elements:
                    # Ensure tuple is complete (key, label, type, options, default)
                    if len(element) < 5:
                        element += ["", ""]  # pad missing fields
                    key, label_text, input_type = element[0], element[1], element[2]
                    options = element[3] if len(element) > 3 else []
                    default_value = element[4] if len(element) > 4 else ""

                    label = QLabel(label_text)
                    label.setObjectName(key + "_label")
                    grid_weld.addWidget(label, r, 1)

                    if input_type == TYPE_TEXTBOX:
                        textbox = QLineEdit()
                        textbox.setObjectName(key)
                        textbox.setFixedSize(130, 22)
                        textbox.setText(str(default_value))
                        grid_weld.addWidget(textbox, r, 2)
                        if key in self.save_changes_list:
                            self.connect_widget_for_change(textbox)
                        ...
                        grid_weld.addWidget(combo, r, 2)
                        if key in self.save_changes_list:
                            self.connect_widget_for_change(combo)

                    r += 1

            elif tab_type == TYPE_TAB_5:
                tab_bolt = QWidget()
                self.tabWidget.addTab(tab_bolt, tab_name)
                tab_index += 1
                self.tabWidget.tabs.setTabText(tab_index, tab_name)
                tab_bolt.setObjectName(tab_name)

                layout_bolt = QVBoxLayout(tab_bolt)

                scroll_bolt = QScrollArea(tab_bolt)
                scroll_bolt.setWidgetResizable(True)
                bolt_contents = QWidget()
                scroll_bolt.setWidget(bolt_contents)
                layout_bolt.addWidget(scroll_bolt)

                vertical_bolt = QVBoxLayout(bolt_contents)
                horizontal_layout_bolt = QHBoxLayout()
                vertical_bolt.addLayout(horizontal_layout_bolt)

                grid_bolt = QGridLayout()
                grid_bolt.setAlignment(Qt.AlignTop | Qt.AlignLeft)
                grid_bolt.setHorizontalSpacing(10)
                grid_bolt.setVerticalSpacing(10)
                horizontal_layout_bolt.addLayout(grid_bolt)

                r = 1
                title = QLabel("Bolt Design Preferences")
                title.setObjectName("_title")
                grid_bolt.addWidget(title, r, 1)
                r += 2
                ...
                for element in elements:
                    ...
                    if input_type == TYPE_TEXTBOX:
                        ...
                        textbox.setFixedSize(130, 22)
                        if input_dictionary:
                            textbox.setText(str(element[4]))
                        grid_bolt.addWidget(textbox, r, 2)
                        if key in self.save_changes_list:
                            self.connect_widget_for_change(textbox)
